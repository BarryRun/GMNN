...
class Trainer(object):
    ...
    def update_soft(self, inputs, target, idx):
        ...
        # print(logits.shape) ==> torch.Size([2708, 7])，即所有数据预测的标签
        # 该公式计算了softmax_cross_entropy,一种损失函数
        loss = -torch.mean(torch.sum(target[idx] * logits[idx], dim=-1))
        # print(target[idx].shape) ==> torch.Size([140, 7])
        # print(logits[idx].shape) ==> torch.Size([140, 7])
        # print(torch.sum(target[idx] * logits[idx], dim=-1).shape) ==> torch.Size([140])
        # print(loss)为一个数值
